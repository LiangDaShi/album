...
function checkRoute ($url)
{
	...
	foreach ($routes as  $route) {
		$need_replace = $route['needreplace'];
		$rule         = $route['regex'];
		if (preg_match($rule , $url , $matches)) {
			foreach ($need_replace as $index => $item) {
				// 去除左边 :
				$item                   = ltrim($item , "\x3A");
				$route['params'][$item] = $matches[$index + 1];
			}
...
function replaceRoute ($app , $action , $params , $extra)
{
	...
	foreach ($routes['data'] as $key => $rule) {
		if ($rule['params']['app'] != $params['app'] && $rule['params']['m'] != $params['m']) {
//如果app及m不匹配直接退出
			continue;
		}
	...
	if ($flag) {
		...
		if (isset($oldparams['id']) && $repalce_arr) {
			//TODO: 可能id不是唯一需要过滤的
			unset($oldparams['id']);
		}
...
function app ($app)
{
	...
	if (!class_exists($appClassName , false)) {
		include_once $path  . strtolower($app) . '.class.php';
	}
	if (!class_exists($appClassName , false)) {
		return false;
	}
...
function meiu_autoload ($className)
{
	...
	if (strtolower(substr($className , 0 , 2)) == 'x_') {
//载入app的lib类,类名必须以x_开头
